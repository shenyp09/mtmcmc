...
def generate_html_report(
    ...
    # 获取HTML模板
    template_text = get_html_template(template_dir)

    # 渲染HTML
    html_content = render_html(template_text, report_data)

    # 写入文件
    output_file = os.path.join(output_dir, "mtmcmc_report.html")
...
def prepare_report_data(
    y, ysigma, templates, templates_sigma, results, fit_stats, figure_files, interactive
):
    """准备报告数据"""
    # 获取参数信息
    params_info = []
    ...
    quality_text = (
        "优秀" if 0.8 <= rchisq <= 1.2 else "一般" if 0.5 <= rchisq <= 1.5 else "较差"
    )
    ...
    if convergence_info["autocorr_time"] is not None:
        ...
        convergence_status = {
            "converged": converged,
            "status": "良好" if converged else "不足",
            "autocorr_text": autocorr_text,
            "threshold_ratio": threshold_ratio,
            "recommendation": "结果可靠" if converged else "建议增加采样步数",
        }
    else:
        convergence_status = {
            "converged": False,
            ...
            "autocorr_text": "无法计算",
            "threshold_ratio": "N/A",
            "recommendation": "建议增加采样步数以获得可靠结果",
        }
...
def get_html_template(template_dir=None):
    """获取HTML模板"""
